...
class NeRFDataset:
    ...
    def collate(self, index):
        ...
        up_vector = torch.tensor([0, 1, 0], dtype=torch.float, device="cuda").expand_as(centers)
        ...
        focal_length_pytorch3d = torch.tensor([focal, focal], dtype=torch.float, device=self.device).unsqueeze(0)
        principal_point_pytorch3d = torch.tensor([64, 64], dtype=torch.float, device=self.device).unsqueeze(0)
        image_size = torch.tensor([128, 128], dtype=torch.int, device=self.device).unsqueeze(0)
        cameras = PerspectiveCameras(
            R=R,
            T=T,
            focal_length=focal_length_pytorch3d,
            principal_point=principal_point_pytorch3d,
