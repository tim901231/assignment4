...
class Gaussians:
    ...
    def _load_random(self, num_points: int):
        ...
        # data["means"] = (torch.rand((num_points, 3)).to(torch.float32) - 0.5) * 1.8  # (N, 3)

        # Initializing opacities such that all when sigmoid is applied to pre_act_opacities,
        # we will have a opacity value close to (but less than) 1.0
        # data["pre_act_opacities"] = 8.0 * torch.ones((num_points,), dtype=torch.float32)  # (N,)
        data["pre_act_opacities"] = 0.3 * torch.ones((num_points,), dtype=torch.float32)  # (N,)
    ...
    def compute_cov_3D(self, quats: torch.Tensor, scales: torch.Tensor):
        ...
        if self.is_isotropic:
            ...
            ### YOUR CODE HERE ###
            cov_3D = (scales.reshape(N, 1, 1)**2) * torch.eye(3).expand(N, 3, 3).to(device=scales.device)

        # HINT: You can use a function from pytorch3d to convert quaternions to rotation matrices.
        else:

            R = quaternion_to_matrix(quats) # N x 3 x 3
            S = (scales.reshape(N, 3, 1)**2) * torch.eye(3).expand(N, 3, 3).to(device=scales.device)

            cov_3D = R @ S @ R.transpose(-2, -1)
    ...
    def evaluate_gaussian_2D(points_2D: torch.Tensor, means_2D: torch.Tensor, cov_2D_inverse: torch.Tensor):
        ...
        P = points_2D.shape[1]
        power = -0.5 * (diff.transpose(-2, -1) @ cov_2D_inverse.unsqueeze(1).expand(-1, P, -1, -1) @ diff).squeeze(-1).squeeze(-1)  # (N, H*W)
...
class Scene:
    ...
    def compute_depth_values(self, camera: PerspectiveCameras):
        ...
        view_transform = camera.get_world_to_view_transform()
        z_vals = view_transform.transform_points(self.gaussians.means)[:, 2]  # (N,)
    ...
    def compute_alphas(self, opacities, means_2D, cov_2D, img_size):
        ...
        ### YOUR CODE HERE ###
        # HINT: Refer to README for a relevant equation.
        alphas = opacities.view(-1, 1) * exp_power  # (N, H*W)
    ...
    def splat(
        ...
        self.visible_num = (opacities.detach() > 0.005).sum().item()
    ...
    def calculate_gaussian_directions(self, means_3D, camera):
        ...
        gaussian_dirs = gaussian_dirs / torch.norm(gaussian_dirs, dim=1, keepdim=True)
